INSERT INTO sop (id, slug, title, version, owner, steps_json, created_at) VALUES
(
  1,
  'incident-generic',
  'Respons insiden generik TechOps',
  1,
  'techops',
  '[
...
(
  2,
  'payment-latency',
  'Investigasi latency / timeout pembayaran',
  1,
  'techops',
  '[
...
INSERT INTO sop_rule (id, sop_id, priority, service, keyword, severity_min) VALUES
...
-- Ensure sequences are advanced when explicit IDs are seeded.
SELECT setval(pg_get_serial_sequence('sop', 'id'), COALESCE((SELECT MAX(id) FROM sop), 1), false);
SELECT setval(pg_get_serial_sequence('sop_rule', 'id'), COALESCE((SELECT MAX(id) FROM sop_rule), 1), false);
